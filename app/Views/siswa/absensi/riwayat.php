...
<div class="card">
    <div class="card-header">
        <div class="card-title">
            ...
            Riwayat: <?= $bulanList[$bulan ?? date('m')] ?? '' ?> <?= $tahun ?? date('Y') ?>
        </div>
                ...
                <table class="table">
                    ...
                    <tbody>
                        <?php $no = 1; foreach ($riwayat as $r): 
                            $label = strtolower($r['status_label'] ?? $r['status'] ?? '');
                            $badge = match($label) {
                                'hadir' => 'badge-success',
                                'izin' => 'badge-warning',
                                'sakit' => 'badge-info',
                            ...
                            $iconMetode = str_contains($metode, 'qr') ? '📱 QR' : (str_contains($metode, 'manual') ? '✍️ Manual' : esc($metode));
                        ?>
                        <tr>
                            <td><?= $no++ ?></td>
                            ...
                            <td><span class="badge <?= $badge ?>"><?= esc($r['status_label'] ?? $r['status'] ?? '-') ?></span></td>
