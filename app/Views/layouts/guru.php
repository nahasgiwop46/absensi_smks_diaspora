...
<head>
    ...
    <title><?= esc($pageTitle ?? 'Dashboard Guru') ?> - Absensi QR SMKS 006</title>
    ...
    <style>
        ...
        * { margin: 0; padding: 0; box-sizing: border-box; }
        ...
        .nav-item.active {
            ...
            color: white; box-shadow: 0 4px 12px rgba(5,150,105,0.4); font-weight: 600;
        }
        ...
        .btn-primary { background: linear-gradient(135deg, var(--primary), var(--primary-dark)); color: white; box-shadow: 0 2px 8px rgba(5,150,105,0.3); }
        .btn-primary:hover { box-shadow: 0 4px 12px rgba(5,150,105,0.4); transform: translateY(-1px); }
        ...
        .form-input, .form-select, .form-textarea { width: 100%; padding: 10px 14px; border: 2px solid var(--border); border-radius: 10px; font-size: 13px; background: #F9FAFB; transition: var(--transition); font-family: inherit; }
        .form-input:focus, .form-select:focus { outline: none; border-color: var(--primary); background: white; box-shadow: 0 0 0 3px rgba(5,150,105,0.1); }
...
<aside class="sidebar" id="sidebar">
    <div class="sidebar-header">
        ...
        <span class="sidebar-title">Guru Panel</span>
    ...
    <nav class="sidebar-nav">
        ...
        <a href="/guru" class="nav-item <?= ($seg2 === '' || $seg2 === 'dashboard') ? 'active' : '' ?>" data-tooltip="Dashboard">
            <i class="fas fa-th-large"></i> <span>Dashboard</span>
        </a>
        
        <div class="nav-section-title">QR & Absensi</div>
        
        <a href="/guru/qrcode" class="nav-item <?= ($seg2 === 'qrcode') ? 'active' : '' ?>" data-tooltip="Generate QR">
            <i class="fas fa-qrcode"></i> <span>Generate QR</span>
        </a>
        <a href="/guru/scan" class="nav-item <?= ($seg2 === 'scan') ? 'active' : '' ?>" data-tooltip="Scan QR Siswa">
            <i class="fas fa-camera"></i> <span>Scan QR Siswa</span>
        </a>
        <a href="/guru/absensi" class="nav-item <?= ($seg2 === 'absensi' && $seg3 !== 'rekap') ? 'active' : '' ?>" data-tooltip="Absensi Manual">
            <i class="fas fa-clipboard-list"></i> <span>Absensi Manual</span>
        </a>
        
        <div class="nav-section-title">Monitoring</div>
        
        <a href="/guru/absensi/rekap" class="nav-item <?= ($seg2 === 'absensi' && $seg3 === 'rekap') ? 'active' : '' ?>" data-tooltip="Rekap Absensi">
            <i class="fas fa-chart-bar"></i> <span>Rekap Absensi</span>
        </a>
        <a href="/guru/siswa" class="nav-item <?= ($seg2 === 'siswa') ? 'active' : '' ?>" data-tooltip="Daftar Siswa">
    ...
    <div class="sidebar-footer">
        <div class="sidebar-user">
            <div class="sidebar-avatar">
                <?= esc(strtoupper(substr(session()->get('nama_lengkap') ?? 'G', 0, 1))) ?>
            </div>
            <div>
                <div class="sidebar-user-name"><?= esc(session()->get('nama_lengkap') ?? 'Guru') ?></div>
                <div class="sidebar-user-role"><?= esc(session()->get('role_nama') ?? 'Guru') ?></div>
            </div>
        </div>
    </div>
</aside>
    ...
    <header class="header" id="header">
        <div class="header-left">
            ...
            <nav class="breadcrumb">
                <a href="/guru"><i class="fas fa-home"></i> Home</a>
        ...
        <div class="header-right">
            <div class="header-dropdown" id="userDropdown" onclick="toggleDropdown()">
                ...
                <div class="header-user-info">
                    ...
                    <span><?= esc(explode(' ', session()->get('nama_lengkap') ?? 'Guru')[0]) ?></span>
                </div>
                <i class="fas fa-chevron-down dropdown-arrow-icon"></i>

                <div class="dropdown-menu">
                    <div class="dropdown-header">
                        <div class="dropdown-user-name"><?= esc(session()->get('nama_lengkap') ?? 'Guru') ?></div>
                        <div class="dropdown-user-role"><?= esc(session()->get('role_nama') ?? 'Guru') ?></div>
                    </div>
                    <a href="/guru/profil" class="dropdown-item"><i class="fas fa-user-circle"></i> Profil Saya</a>
...
<script>
    let sidebarCollapsed = localStorage.getItem('guruSidebarCollapsed') === 'true';
